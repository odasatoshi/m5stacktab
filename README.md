...
カメラ        : SC202CS (MIPI-CSI 1 レーン) が載っているがファームは未使用 (#83 で撤去)
               駆動したときの知見は CLAUDE.md に残してある
...
メニューの `WiFi` に保存済みが並ぶ（**最大 5 件**、`*` が今つながっているもの）。
...
メニューは `SSH` / `VPN` / `WiFi` / `Miscellanea` の 4 つで、**接続先の 3 つは同じ形**
にしてある (#82) — 一覧 → 選ぶと `接続` / `削除`、末尾の `Create new ...` で新規作成に入る。

新規作成は 1 項目ずつ聞く。行を選ぶと、文字の項目は端末に戻って 1 行入力になり、
`種別` / `key` / `via` は押すたびに候補を回す（鍵は NVS にあるものから選ぶ）。
`保存` で NVS に足す。

- **書式の検査は保存のときにパーサ (`prof::parse`) へ通してやる。** 画面側に検査を
  二重に持たない（食い違うと「保存はできたのに一覧に出ない」になる）
- **同じ `name` があると断る。** 上書きはしない（消してから作り直す）
- **編集の途中で Esc を押すと書きかけを捨てて端末に戻る**（打ち直し）
- 鍵は端末では作れない。`profiles import` で取り込んだものから選ぶ
