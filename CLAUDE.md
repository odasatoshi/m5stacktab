...
## 実機を指で触らないと出てこない数字がある

**コンソールのコマンドで代用した検証は、走るタスクが違う。**
`menu enter` / `tap` / `kbdinject` はどれもコンソールタスク (32KB) の上で動くが、
**実際の指はメインループ (8192 → 16384) の上**にいる。スタックの残量を測ると、
同じ操作でも桁が変わる:

```
menu enter（コンソール経由） → stack headroom 27484 bytes
指でタップ（メインループ）   → stack headroom  1660 bytes   ← #73 で実測
```

**平常時ですら 3200 バイトしか余っていない**ので、メインループで cJSON のような
ものを通すと危ない。`CONFIG_ESP_MAIN_TASK_STACK_SIZE` を 16384 にしてある
（ワーカタスクに逃がしても平常時の細さは変わらないため、ここを広げる方を採った）。

測り方は `uxTaskGetStackHighWaterMark(nullptr)` を**その処理の中で**呼ぶ。
**高水位マークは残り続ける**ので、指で操作した後にコンソールから読み出せばよい。
